...
export function createRuleForm({ mount, onChange }) {
  ...
  const bodyWrap=document.createElement('label'); bodyWrap.className='editor-field'; bodyWrap.innerHTML='<span>body</span>'; const body=document.createElement('textarea'); body.addEventListener('input',()=>{state.body=body.value;onChange();}); bodyWrap.append(body); mount.append(bodyWrap);
