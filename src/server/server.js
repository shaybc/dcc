...
function incrementDefinitionVersion(versionValue) {
  const raw = String(versionValue || "").trim();
  if (!raw) {
    return "0.1";
  }
  if (/^\d+\.\d+$/.test(raw)) {
    return (Number(raw) + 0.1).toFixed(1);
  }
...
function updateVersionInDefinitionContent(content, filePath) {
  const ext = path.extname(filePath).toLowerCase();
  if ([".md", ".markdown"].includes(ext)) {
    const parsed = matter(content || "");
    const nextVersion = incrementDefinitionVersion(parsed.data?.version);
    parsed.data = { ...parsed.data, version: nextVersion };
    return { content: matter.stringify(parsed.content || "", parsed.data), version: nextVersion };
  }

  let yamlDoc = {};
  try {
    yamlDoc = YAML.parse(content || "") || {};
  } catch (_error) {
    yamlDoc = {};
  }
  const nextVersion = incrementDefinitionVersion(yamlDoc?.version);
  yamlDoc.version = nextVersion;
  return { content: YAML.stringify(yamlDoc), version: nextVersion };
}

function ensureFileNameForType(fileName, normalizedType) {
  const base = sanitizeDuplicateFileName(fileName);
  if (!base) {
    return "";
  }
  ...
  if (["agents", "rules"].includes(normalizedType)) {
    return `${base}.md`;
  }
  return `${base}.yaml`;
}

app.post("/api/definitions/parse", async (req, res) => {
  try {
    const normalizedType = normalizeDefinitionType(req.body?.type);
    ...
    if (["agents", "rules"].includes(normalizedType)) {
      ...
      schema = String(parsed.data?.schema || "");
      description = String(parsed.data?.description || "");
      tags = normalizeTags(parsed.data?.tags || []);
      fields.body = String(parsed.content || "").trim();
      if (normalizedType === "agents") {
        fields.tools = String(parsed.data?.tools || "");
        fields.rules = String(parsed.data?.rules || "");
      }
    } else {
      ...
      if (normalizedType === "prompts") {
        const prompt = Array.isArray(doc.prompts) ? doc.prompts[0] || {} : {};
        fields.itemName = String(prompt.name || "");
        fields.itemDescription = String(prompt.description || "");
        fields.itemBody = String(prompt.prompt || "");
      }
      if (normalizedType === "mcp servers") {
        const item = Array.isArray(doc.mcpServers) ? doc.mcpServers[0] || {} : {};
        fields.itemName = String(item.name || "");
        fields.command = String(item.command || "");
        fields.args = Array.isArray(item.args) ? item.args.join(", ") : "";
      }
      if (normalizedType === "models") {
        const model = Array.isArray(doc.models) ? doc.models[0] || {} : {};
        fields.itemName = String(model.name || "");
        fields.provider = String(model.provider || "");
        fields.modelId = String(model.model || "");
        fields.apiKey = String(model.apiKey || "");
        fields.roles = Array.isArray(model.roles) ? model.roles.join(", ") : "";
        fields.contextLength = String(model.defaultCompletionOptions?.contextLength || "");
      }
      if (normalizedType === "workflows") {
        fields.modelsYaml = YAML.stringify(doc.models || []).trimEnd();
        fields.contextYaml = YAML.stringify(doc.context || []).trimEnd();
        fields.mcpYaml = YAML.stringify(doc.mcpServers || []).trimEnd();
        fields.rulesYaml = YAML.stringify(doc.rules || []).trimEnd();
      }
      if (normalizedType === "context") {
        const item = Array.isArray(doc.context) ? doc.context[0] || {} : {};
        fields.provider = String(item.provider || "");
        fields.url = String(item.params?.url || "");
      }
    }

    res.json({ name, version, schema, description, tags, fields });
  } catch (error) {
    res.status(400).json({ error: error.message || "Unable to parse definition content." });
  }
});

app.post("/api/definitions/:id/edit", async (req, res) => {
  ...
  try {
    ...
    const updated = updateVersionInDefinitionContent(incomingContent, absoluteDefinitionPath);
    await fsp.writeFile(absoluteDefinitionPath, updated.content, "utf8");
    await runCommand(`git add ${JSON.stringify(relativePath)}`, { cwd: absoluteRepoPath });
    await runCommand(`git commit -m ${JSON.stringify(`Edit definition ${row.name} v${updated.version}`)}`, { cwd: absoluteRepoPath });
    await runCommand("git push", { cwd: absoluteRepoPath });
    await loadDefinitions();
    res.json({ ok: true, version: updated.version, message: `Definition saved to repository as version ${updated.version}.` });
  } catch (error) {
    res.status(500).json({ error: extractCommandErrorMessage(error, "Unable to save definition edits.") });
  }
});
...
app.post("/api/definitions/create-local", async (req, res) => {
  try {
    ...
    const normalizedType = normalizeDefinitionType(req.body?.type);
    const fileName = ensureFileNameForType(req.body?.fileName, normalizedType);
    if (!fileName) {
      res.status(400).json({ error: "File name is required." });
      return;
    }

    const directoryPathRaw = String(req.body?.directoryPath || "").trim();
    if (!directoryPathRaw) {
    ...
    const targetDir = path.resolve(path.isAbsolute(directoryPathRaw) ? directoryPathRaw : path.join(baseRepoPath, directoryPathRaw));
    ...
    await loadDefinitions();
    res.json({ ok: true, filePath: targetPath, message: "Definition saved as local untracked file." });
  } catch (error) {
