...
function sanitizeRelativeDestination(destinationPath) {
  const rawValue = String(destinationPath || "").trim();
  if (!rawValue) {
    return "";
  }
...
function incrementMinorVersion(versionValue) {
  const parsed = Number(String(versionValue || "0").trim());
...
function bumpVersionInContent(content, filePath) {
  const ext = path.extname(String(filePath || "")).toLowerCase();
  const rawContent = String(content || "");
  const yamlVersionRegex = /^(\s*version\s*:\s*)([^\n]+)$/m;

  if ([".yml", ".yaml"].includes(ext)) {
    if (yamlVersionRegex.test(rawContent)) {
      return rawContent.replace(yamlVersionRegex, (_match, prefix, value) => `${prefix}${incrementMinorVersion(value)}`);
    }
    return `version: 0.1\n${rawContent}`;
  }
  ...
  if ([".md", ".markdown"].includes(ext)) {
    ...
    const bumpedFrontmatter = yamlVersionRegex.test(frontmatter)
      ? frontmatter.replace(yamlVersionRegex, (_match, prefix, value) => `${prefix}${incrementMinorVersion(value)}`)
      : `version: 0.1\n${frontmatter}`;
    return rawContent.replace(frontmatter, bumpedFrontmatter);
  }
...
app.post("/api/definitions/create", async (req, res) => {
  try {
    const repoPath = await getSetting("repoPath");
    if (!repoPath) {
      res.status(400).json({ error: "Repo path not configured." });
    ...
    const relativePath = sanitizeRelativeDestination(req.body?.relativePath);
    const content = String(req.body?.content || "");

    if (!relativePath) {
      res.status(400).json({ error: "A valid destination file path is required." });
      return;
    }

    const absoluteRepoPath = path.resolve(repoPath);
    const absolutePath = path.resolve(absoluteRepoPath, relativePath);
    if (!absolutePath.startsWith(`${absoluteRepoPath}${path.sep}`)) {
      res.status(400).json({ error: "Destination path must remain inside the repository." });
      return;
    }
    ...
    const createdType = path.basename(path.dirname(absolutePath)).toLowerCase();
    const createdKey = buildKey(createdType, absolutePath);
    const createdRow = await getDb("SELECT id FROM definitions WHERE key = ?", [createdKey]);
    res.json({ ok: true, id: createdRow?.id || null, message: "Definition created as local untracked file." });
  } catch (error) {
    res.status(500).json({ error: error.message || "Failed to create definition." });
  }
});
...
app.post("/api/definitions/:id/edit", async (req, res) => {
  db.get("SELECT * FROM definitions WHERE id = ?", [req.params.id], async (err, row) => {
    ...
    try {
      ...
      const absoluteDefinitionPath = path.resolve(row.filePath || "");
      if (!absoluteDefinitionPath.startsWith(`${absoluteRepoPath}${path.sep}`)) {
        res.status(400).json({ error: "Definition file is outside the configured repository." });
        return;
      }

      const updatedContent = String(req.body?.content || "");
      const commitMessage = String(req.body?.commitMessage || "").trim() || `Edit definition ${row.name}`;
      const relativePath = path.relative(absoluteRepoPath, absoluteDefinitionPath);
      const bumpedContent = bumpVersionInContent(updatedContent, absoluteDefinitionPath);

      await runCommand("git pull", { cwd: absoluteRepoPath });
      await fsp.writeFile(absoluteDefinitionPath, bumpedContent, "utf8");
      await runCommand(`git add ${JSON.stringify(relativePath)}`, { cwd: absoluteRepoPath });
      await runCommand(`git commit -m ${JSON.stringify(commitMessage)}`, { cwd: absoluteRepoPath });
      await runCommand("git push", { cwd: absoluteRepoPath });
      await loadDefinitions();
      res.json({ ok: true, message: "Definition edit saved and pushed to repository." });
    } catch (error) {
      res.status(500).json({ error: extractCommandErrorMessage(error, "Failed to edit definition.") });
    }
  });
});
