...
function incrementSemanticMinorByTenth(version) {
  const parsed = Number.parseFloat(String(version || "0").trim());
  const safe = Number.isFinite(parsed) ? parsed : 0;
  return (Math.round((safe + 0.1) * 10) / 10).toFixed(1);
}

function updateVersionInContent(filePath, content, nextVersion) {
  ...
  const matterDoc = matter(content || "");
  matterDoc.data = matterDoc.data || {};
  matterDoc.data.version = nextVersion;
  return matter.stringify(matterDoc.content || "", matterDoc.data);
}

function parseDefinitionForForm(type, content) {
  const normalizedType = normalizeDefinitionType(type);
  if (["agents", "rules"].includes(normalizedType)) {
    ...
    return {
      ...
      tools: normalizeTags(parsed.data?.tools),
      rules: normalizeTags(parsed.data?.rules),
      body: parsed.content || ""
    };
  }
  ...
  if (normalizedType === "prompts") base.promptsYaml = YAML.stringify(parsed.prompts || []).trim();
  if (normalizedType === "mcpservers") base.mcpServersYaml = YAML.stringify(parsed.mcpServers || []).trim();
  if (normalizedType === "models") base.modelsYaml = YAML.stringify(parsed.models || []).trim();
  if (normalizedType === "context") base.contextYaml = YAML.stringify(parsed.context || []).trim();
  if (normalizedType === "workflows") {
    base.modelsYaml = YAML.stringify(parsed.models || []).trim();
    base.contextYaml = YAML.stringify(parsed.context || []).trim();
    base.mcpServersYaml = YAML.stringify(parsed.mcpServers || []).trim();
    base.rulesYaml = YAML.stringify(parsed.rules || []).trim();
  }
  return base;
}
...
app.post("/api/definitions/parse-form", async (req, res) => {
  try {
    const type = req.body?.type || "";
    const content = String(req.body?.content || "");
    const data = parseDefinitionForForm(type, content);
    res.json({ ok: true, data });
  } catch (error) {
    res.status(400).json({ error: error.message || "Unable to parse content." });
  }
});
...
app.post("/api/definitions/create-local", async (req, res) => {
  try {
    ...
    const saveDir = path.resolve(String(req.body?.saveDir || repoPath));
    ...
    res.json({ ok: true, message: "Definition saved as local untracked file." });
  } catch (error) {
    res.status(500).json({ error: error.message || "Unable to create definition." });
  }
});
...
app.post("/api/definitions/:id/edit", async (req, res) => {
  ...
  if (!absoluteDefinitionPath.startsWith(`${absoluteRepoPath}${path.sep}`)) {
    res.status(400).json({ error: "Definition file is not in configured repository." });
    return;
  }

  const inputContent = String(req.body?.content || "");
  const nextVersion = incrementSemanticMinorByTenth(row.version || "0.0");
  const nextContent = updateVersionInContent(row.filePath, inputContent, nextVersion);
  ...
  try {
    ...
    await runCommand(`git commit -m ${JSON.stringify(`Edit definition ${row.name} v${nextVersion}`)}`, { cwd: absoluteRepoPath });
    await runCommand("git push", { cwd: absoluteRepoPath });
    await loadDefinitions();
    res.json({ ok: true, version: nextVersion, message: "Definition edit saved and pushed to repository." });
  } catch (error) {
    res.status(500).json({ error: extractCommandErrorMessage(error, "Unable to save definition edit.") });
  }
});
